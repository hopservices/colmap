...
void print_help(char* program_name) {
  printf(
      ...
      "[options] graph_file number_of_clusters\n",
      program_name);
  ...
  // printf("          -s use spectral at coarsest level (default is METIS
  // init.)\n");
  printf(
      "\nTo compute objective function value for a given clustering:\n %s "
      "[options] -e clustering_file graph_file\n\n",
      program_name);
}
...
void clusterSize(GraphType* graph, int* clustersize) {
  ...
  for (i = 0; i < nvtxs; i++) clustersize[where[i]]++;
}
...
void sparse2dense(GraphType* graph, double* dense, float* m_adjwgt) {
  ...
  for (i = 0; i < nvtxs; i++)
    for (j = 0; j < nvtxs; j++) dense[i * nvtxs + j] = 0;

  if (adjwgt == NULL)
    for (i = 0; i < nvtxs; i++)
      for (j = xadj[i]; j < xadj[i + 1]; j++) dense[i * nvtxs + adjncy[j]] = 1;
  else
    for (i = 0; i < nvtxs; i++)
...
void extractfilename(char* path, char* name) {
  ...
  for (i = length - 1; i >= 0; i--)
    if ((path[i] == '/') || (path[i] == '\\')) {
      i++;
      for (j = i; j < length; j++) name[j - i] = path[j];
      name[j - i] = '\0';
      break;
    } else if (i == 0) {
      for (j = i; j < length; j++) name[j - i] = path[j];
...
float** f2malloc(int n, int m, char* msg) {
  ...
  for (i = 0; i < n; i++) temp[i] = (float*)GKmalloc(sizeof(float) * m, msg);
  return temp;
}
...
int** i2malloc(int n, int m, char* msg) {
  ...
  for (i = 0; i < n; i++) temp[i] = (int*)GKmalloc(sizeof(int) * m, msg);
  return temp;
}
...
void* GKmalloc(int nbytes, char* msg) {
  ...
  if (ptr == NULL)
    graclus_errexit(
        "***Memory allocation failed for %s. Requested size: %d bytes", msg,
        nbytes);
...
int iamax(int n, int* x) {
  ...
  for (i = 1; i < n; i++) max = (x[i] > x[max] ? i : max);
...
int idxamax_strd(int n, idxtype* x, int incx) {
  ...
  n *= incx;
  for (i = incx; i < n; i += incx) max = (x[i] > x[max] ? i : max);
...
int idxamin(int n, idxtype* x) {
  ...
  for (i = 1; i < n; i++) min = (x[i] < x[min] ? i : min);

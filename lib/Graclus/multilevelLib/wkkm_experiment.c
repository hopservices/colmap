 ...
 */
{
  ...
  if ((cutType == RASSO) || (cutType == RCUT))
    for (i = 0; i < nvtxs; i++) w[i] = 1;
  else if (adjwgt == NULL)
    for (i = 0; i < nvtxs; i++)
      for (j = xadj[i]; j < xadj[i + 1]; j++) w[i]++;
  else
    for (i = 0; i < nvtxs; i++)
      for (j = xadj[i]; j < xadj[i + 1]; j++) w[i] += adjwgt[j];
}

void transform_matrix(CtrlType* ctrl, GraphType* graph, idxtype* w,
                      float* m_adjwgt)
/* normalized the adjacency matrix for Ncut only*/
{
  ...
  if (cutType == RASSO) {  // ratio asso.
    if (adjwgt == NULL)
      for (i = 0; i < nvtxs; i++)
        for (j = xadj[i]; j < xadj[i + 1]; j++) m_adjwgt[j] = 1;
    else
      for (i = 0; i < nvtxs; i++)
        for (j = xadj[i]; j < xadj[i + 1]; j++) m_adjwgt[j] = adjwgt[j];
  } else {  // normalize rows and columns
    if (adjwgt == NULL) {
      for (i = 0; i < nvtxs; i++)
        for (j = xadj[i]; j < xadj[i + 1]; j++)
          if (w[i] > 0) m_adjwgt[j] = 1.0 / w[i];
      for (i = 0; i < nvtxs; i++)
        for (j = xadj[i]; j < xadj[i + 1]; j++)
          if (w[i] > 0) m_adjwgt[j] /= w[adjncy[j]];
    } else {
      for (i = 0; i < nvtxs; i++)
        for (j = xadj[i]; j < xadj[i + 1]; j++)
          if (w[i] > 0)
...
void pingpong(CtrlType* ctrl, GraphType* graph, int nparts, int chain_length,
              float* tpwgts, float ubfactor, int toplevel)
// do batch-local search; chain_length is the search length
{
  ...
  do {
    // Weighted_kernel_k_means(ctrl, graph, nparts, w, m_adjwgt, tpwgts,
    // ubfactor);
    Weighted_kernel_k_means(ctrl, graph, nparts, w, tpwgts, ubfactor);
    if (chain_length > 0) {
      // moves = local_search(ctrl, graph, nparts, chain_length, w, m_adjwgt,
      // tpwgts, ubfactor);
      moves =
...
void Weighted_kernel_k_means(CtrlType* ctrl, GraphType* graph, int nparts,
                             idxtype* w, float* tpwgts, float ubfactor) {
  // w is the weights

  int nvtxs, nbnd, nedges, me, i, j;
  idxtype *squared_sum, *sum, *xadj, *adjncy, *adjwgt, *where, *new_where,
      *bndptr, *bndind;
  ...
  // we need new_where because in kernel k-means distance is based on cluster
  // label if we change a label, distance to that cluster will change
  new_where = imalloc(nvtxs, "Weighted_kernel_k_means: new_where");
  for (i = 0; i < nvtxs; i++) new_where[i] = where[i];

  sum = idxsmalloc(nparts, 0, "Weighted_kernel_k_means: weight sum");
  inv_sum = fmalloc(nparts, "Weighted_kernel_k_means: sum inverse");
  squared_inv_sum =
      fmalloc(nparts, "Weighted_kernel_k_means: squared sum inverse");
  squared_sum =
      idxsmalloc(nparts, 0, "Weighted_kernel_k_means: weight squared sum");

  // initialization

  obj = old_obj = 0;
  for (i = 0; i < nvtxs; i++) sum[where[i]] += w[i];
  for (i = 0; i < nparts; i++)
    if (sum[i] > 0) {
      inv_sum[i] = 1.0 / sum[i];
  ...
  for (i = 0; i < nvtxs; i++) {
    me = where[i];
    for (j = xadj[i]; j < xadj[i + 1]; j++)
      if (where[adjncy[j]] == me) squared_sum[me] += adjwgt[j];
  }

  // note the obj here is not obj. fun. of wkkm, just the second trace value to
  // be maximized
  for (i = 0; i < nparts; i++)
  ...
  do {
    ...
    /*
    if (adjwgt == NULL) // if graph has uniform edge weights
      for (i=0; i<nvtxs; i++){ // compute linear term in distance from point i
    to all centers min_ind = me = where[i];

        for (k=0; k<nparts; k++)
          linearTerm[k] =0;
        for (j=xadj[i]; j<xadj[i+1]; j++)
          linearTerm[where[adjncy[j]]] ++; // this line is the only difference
    between if and else for (k=0; k<nparts; k++) printf("U%f ",
    linearTerm[k]*1.0/w[i]); printf("\n"); min_dist =
    squared_sum[me]*w[i]-2*linearTerm[me]*sum[me];
        ...
        if(me != min_ind){
          new_where[i] = min_ind; // note here we can not change where;
    otherwise we change the center change ++;
        }
      }
    ...
    */
    ...
    for (ii = 0; ii < loopend; ii++) {
      ...
      if (w[i] > 0) {
        ...
        for (k = 0; k < nparts; k++) linearTerm[k] = 0;
        for (j = xadj[i]; j < xadj[i + 1]; j++)
          linearTerm[where[adjncy[j]]] +=
              adjwgt[j];  // only difference between if and else

        min_ind = me = where[i];
        min_dist = squared_sum[me] * squared_inv_sum[me] -
                   2 * inv_wi * linearTerm[me] * inv_sum[me];
        /*if (sum[me] >0)
          min_dist =
          squared_sum[me]*1.0/(1.0*sum[me]*sum[me])-2.0*linearTerm[me]/(sum[me]*w[i]);
        */
        for (k = 0; k < me; k++) {
          dist = squared_sum[k] * squared_inv_sum[k] -
                 2 * inv_wi * linearTerm[k] * inv_sum[k];
          /*
            dist = 0;
            if (sum[k] >0)
            dist =
            squared_sum[k]*1.0/(1.0*sum[k]*sum[k])-2.0*linearTerm[k]/(sum[k]*w[i]);
          */
          if (dist < min_dist) {
            // if(dist < min_dist){
            min_dist = dist;
        ...
        if (me != min_ind) {
          new_where[i] = min_ind;  // note here we can not change where;
                                   // otherwise we change the center
          change++;
        }
        // if(i==0)
        // printf("%d(%d->%d), sum=(%d, %d), linear=(%f, %f), square=(%d, %d),
        // dis=(%f-%f, %f-%f)\n",i, me, min_ind, sum[me], sum[min_ind],
        // linearTerm[me]*1.0/w[i],linearTerm[min_ind]*1.0/w[i],squared_sum[me],
        // squared_sum[min_ind],
        // squared_sum[me]*1.0/(1.0*sum[me]*sum[me]), 2.0*linearTerm[me]/(sum[me]*w[i]),
    ...
    for (i = 0; i < nparts; i++) sum[i] = squared_sum[i] = 0;
    for (i = 0; i < nvtxs; i++) sum[new_where[i]] += w[i];
    for (i = 0; i < nparts; i++)
      if (sum[i] > 0) {
        inv_sum[i] = 1.0 / sum[i];
        squared_inv_sum[i] = inv_sum[i] * inv_sum[i];
    ...
    for (i = 0; i < nvtxs; i++) {
      me = new_where[i];
      for (j = xadj[i]; j < xadj[i + 1]; j++)
        if (new_where[adjncy[j]] == me) squared_sum[me] += adjwgt[j];
    }
  ...
  free(linearTerm);
  free(inv_sum);
  free(squared_inv_sum);
}

/*
int local_search(CtrlType *ctrl, GraphType *graph, int nparts, int chain_length,
idxtype *w, float *tpwgts, float ubfactor)
     //return # of points moved
{
  ...
  for (k=0; k<nparts; k++)
    if (sum[k] >0)
      //for (i=0; i<nvtxs; i++)
      for (ii=0; ii<nbnd; ii++)
        //kDist[i][k] = squared_sum[k]/(1.0*sum[k]*sum[k]) -
2*kDist[i][k]/sum[k]; kDist[ii][k] = squared_sum[k]/(1.0*sum[k]*sum[k]) -
2*kDist[ii][k]/sum[k];
  ...
  for (i=0; i<chain_length; i++)
    {
      ...
      for (ii=0; ii<nbnd; ii++){
        ...
        if (mark[ii] ==0){
          me = where[j];
          if (sum[me] > w[j]) // if this cluster where j belongs is not a
singleton for (k=0; k<nparts; k++) if (k != me){
                //tempchange = -kDist[j][me]*sum[me]*w[j]/(sum[me]-w[j]) +
kDist[j][k]*sum[k]*w[j]/(sum[k]+w[j]); tempchange =
-kDist[ii][me]*sum[me]*w[j]/(sum[me]-w[j]) +
kDist[ii][k]*sum[k]*w[j]/(sum[k]+w[j]); if (tempchange < tempMinChange){
...
float onePoint_move(GraphType* graph, int nparts, idxtype* sum,
                    idxtype* squared_sum, idxtype* w, idxtype* self_sim,
                    int** linearTerm, int ii) {
  int k, j, s, i, nbnd, temp, q1, q2, minchange_id, new_squared_sum1,
      new_squared_sum2, me, nedges, nvtxs;
  ...
  if (sum[me] > w[s]) {  // if this cluster where j belongs is not a singleton
    ...
    for (k = 0; k < me; k++) {
      ...
      if (sum[k] > 0)
        tempchange = squared_sum[me] * inv_sum1 +
                     squared_sum[k] * 1.0 / sum[k] - q1 * inv_sum2 -
                     q2 * 1.0 / (sum[k] + w[s]);
...
void move1Point2EmptyCluster(GraphType* graph, int nparts, idxtype* sum,
                             idxtype* squared_sum, idxtype* w,
                             idxtype* self_sim, int** linearTerm, int k) {
  int j, s, ii, nbnd, q1, q2, minchange_id, new_squared_sum1, new_squared_sum2,
      me, nvtxs, loopend;
...
// return # of points moved
{
  int nvtxs, nedges, nbnd, me, i, j, k, s, ii, loopend;
  idxtype *sum, *squared_sum, *xadj, *adjncy, *adjwgt, *where, *bndptr, *bndind,
      *self_sim;
  ...
  // EDIT: malloc to nvtxs --Brian
  self_sim = idxsmalloc(nvtxs, 0, "Local_search: self similarity");
  linearTerm = i2malloc(nvtxs, nparts, "Local_search: linear term");
  ...
  // EDIT here from nbnd to nvtxs
  for (ii = 0; ii < nvtxs; ii++)
    for (j = 0; j < nparts; j++) linearTerm[ii][j] = 0;
  ...
  for (ii = 0; ii < loopend; ii++) {
    ...
    for (j = xadj[s]; j < xadj[s + 1]; j++) {
      // kDist[i][where[adjncy[j]]] += 1.0*adjwgt[j]/w[i];
      linearTerm[ii][where[adjncy[j]]] += adjwgt[j];
      if (adjncy[j] == s) self_sim[ii] = adjwgt[j];
    }
  }
  ...
  while (loopTimes < chain_length) {
    ...
    if (boundary_points != 1) {
      for (j = 0; j < nvtxs; j++)
        minchange = onePoint_move(graph, nparts, sum, squared_sum, w, self_sim,
                                  linearTerm, j);
    } else {
      for (ii = 0; ii < nbnd; ii++) {
        minchange = onePoint_move(graph, nparts, sum, squared_sum, w, self_sim,
                                  linearTerm, bndind[ii]);
      }
...
void remove_empty_clusters_l1(CtrlType* ctrl, GraphType* graph, int nparts,
                              idxtype* w, float* tpwgts, float ubfactor) {
  int* clustersize = imalloc(nparts, "remove_empty_clusters: clusterSize");
  int number_of_empty_cluster = 0, i, s;

  for (i = 0; i < nparts; i++) clustersize[i] = 0;
  clusterSize(graph, clustersize);
  for (i = 0; i < nparts; i++)
    if (clustersize[i] == 0) number_of_empty_cluster++;
...
void remove_empty_clusters_l2(CtrlType* ctrl, GraphType* graph, int nparts,
                              idxtype* w, float* tpwgts, float ubfactor) {
  int* clustersize = imalloc(nparts, "remove_empty_clusters: clusterSize");
  int number_of_empty_cluster = 0, i, s;
  ...
  if (number_of_empty_cluster > 0) {
    int nvtxs, me, j, k, ii, loopend;
    idxtype *sum, *squared_sum, *xadj, *adjncy, *adjwgt, *where, *bndptr,
        *bndind, *self_sim, nbnd;
    int** linearTerm;
    ...
    for (k = 0; k < nparts; k++)
      if (clustersize[k] == 0) {
        move1Point2EmptyCluster(graph, nparts, sum, squared_sum, w, self_sim,
                                linearTerm, k);
      }
    free(sum);
    free(squared_sum);
...
void MLKKMRefine(CtrlType* ctrl, GraphType* orggraph, GraphType* graph,
                 int nparts, int chain_length, float* tpwgts, float ubfactor) {
  ...
  for (i = 0;; i++) {
    ...
    */
    ...
    GKfree((void**)&graph->gdata,
           LTERM); /* Deallocate the graph related arrays */
    graph = graph->finer;
  ...
  if (mustfree) GKfree((void**)&graph->vwgt, (void**)&graph->adjwgt, LTERM);
